...
void AsyncWinHttp::Cleanup()
{
	...
	//if (RequesterStatusCallback_ != NULL)
	if (WinHttpAsyncResult_ != NULL)
	{
		//RequesterStatusCallback_(this);
		WinHttpAsyncResult_(this);
		WinHttpAsyncResult_ = NULL;
...
{
	WCHAR szBuffer[256] = {0};
	swprintf_s(szBuffer, L"%s: DATA_AVAILABLE (%d)", this->memo_, lpvStatusInformation);
...
{
	WCHAR szBuffer[256] = {0};	
	swprintf_s( szBuffer,L"%s: REQUEST_ERROR - error %d, result %s", this->memo_, pAsyncResult->dwError,  this->GetApiErrorString(pAsyncResult->dwResult));
	...
	) 
{
	...
	case WINHTTP_CALLBACK_STATUS_REDIRECT:
		swprintf(szBuffer,L"%s: REDIRECT (%d)", pRequester->memo_, dwStatusInformationLength);
		break;
	case WINHTTP_CALLBACK_STATUS_REQUEST_ERROR:
		pRequester->OnRequestError((WINHTTP_ASYNC_RESULT *)lpvStatusInformation);
		break;
	default:
		swprintf(szBuffer, L"%s: Unknown/unhandled callback - status %d given", pRequester->memo_, dwInternetStatus);
		break;
	}
}
